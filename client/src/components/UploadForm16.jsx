  ...
  const handleConfirm = async (e) => {
    ...
    try {
      ...
      try {
        ...
        response = await fetch(`${import.meta.env.VITE_API_URL}/api/calculate-tax`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        console.warn(`Direct calculation to ${import.meta.env.VITE_API_URL} failed, trying Vite proxy fallback`, directErr);
        // Fallback to proxy
      ...
      if (err.name === 'TypeError' || err.message?.includes('fetch') || err.message?.includes('NetworkError') || err.message?.includes('Failed to fetch')) {
        setCalcError('Unable to connect to the server. Please try again later.');
      } else {
  ...
  const handleUploadAndExtract = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(`Direct upload to ${import.meta.env.VITE_API_URL} failed, trying Vite proxy fallback`, directErr);
        // Fallback to proxy
          ...
          <div className="mt-xl border-t border-outline-variant/30 pt-lg space-y-md">
            ...
            {calcResults && (
              <div className="mt-xl border-t border-outline-variant/30 pt-lg space-y-lg">
                ...
                <div className="bg-[#10B981] p-lg rounded-2xl text-white flex flex-col sm:flex-row gap-md justify-between items-start sm:items-center">
                  <div>
                  ...
                  <div className="flex items-center gap-sm justify-between sm:justify-end w-full sm:w-auto">
                    {user && (
                      <>
                        {saveError && (
